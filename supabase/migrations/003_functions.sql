...
BEGIN
    ...
    AND (
        ...
        OR array_length(user_interests & au.interests, 1) > 0
    )
    ORDER BY 
        CASE WHEN user_interests IS NOT NULL AND au.interests IS NOT NULL 
             THEN array_length(user_interests & au.interests, 1) 
             ELSE 0 
        END DESC,
        au.connected_at ASC
    LIMIT 10;
...
-- Function to create or join a chat session
...
BEGIN
    ...
    -- Look for existing waiting session with compatible interests
    SELECT cs.id INTO waiting_session_id
    FROM chat_sessions cs
    JOIN anonymous_users au ON cs.user1_id = au.id
    WHERE cs.status = 'waiting' 
    AND cs.user2_id IS NULL
    AND cs.user1_id != current_user_id
    AND au.is_active = true
    AND NOT EXISTS (
        SELECT 1 FROM banned_users bu 
    ...
    IF waiting_session_id IS NOT NULL THEN
        ...
        UPDATE chat_sessions 
        SET user2_id = current_user_id, status = 'active', started_at = NOW()
        WHERE id = waiting_session_id;
        
        RETURN waiting_session_id;
    ELSE
        -- Create new waiting session
        INSERT INTO chat_sessions (user1_id) 
